...
export function buildConcentrationEgales(
  ...
  let giniDetail = '';
  if (indiceGini < 0.3) giniDetail = `IG = ${indiceGini} → Faible concentration : répartition relativement égalitaire.`;
  else if (indiceGini < 0.6) giniDetail = `IG = ${indiceGini} → Concentration moyenne : inégalités modérées.`;
  else giniDetail = `IG = ${indiceGini} → Forte concentration : une petite partie détient une grande part de la masse.`;
